...
def main():
    ...
    # batch predict needs features; keep target if present for optional eval
    ...
    runs = client.search_runs(
        ...
        filter_string="attributes.run_name = 'profit_linear_regression_baseline'",
        order_by=["attributes.start_time DESC"],
        max_results=1,
    )
    if not runs:
        raise RuntimeError("No run found for profit_linear_regression_baseline. Train first.")
    ...
    model_name = "profit_linear_regression_baseline"
    ...
    with mlflow.start_run(run_name="batch_predict_profit_1m"):
        ...
        tmp_csv = "batch_pred_profit_1m.csv"
